...
for line in lines:
    ...
    if all(component <= limit for rgb in rgbs for component, limit in zip_longest(rgb, (12, 13, 14))):
        total += int(game)

    min_rgb = reduce(lambda rgb1, rgb2: [max(p) for p in zip_longest(rgb1, rgb2)], rgbs)
